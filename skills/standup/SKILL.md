...
EXPORT_FILE="$EXPORT_DIR/standup-$EXPORT_TIMESTAMP.html"
```

### 5-2: HTML ファイルを生成する

Step 2 で作成したマークダウンレポート（`$STANDUP_REPORT`）を HTML に変換して書き出します。
`python3` を使って変換します：
...
export_file = os.environ.get('EXPORT_FILE', '/tmp/standup-report.html')
