...
現在のブランチが $ARGUMENTS でない場合は切り替える。切り替え前に未コミットの変更がないか確認する：
```bash
git status --porcelain
```
未コミットの変更がある場合は一時退避してから切り替える：
```bash
git stash
...
- テストコマンドが見つからない場合: `NO_TEST` ステータスで報告する（PASS 扱いにしない）
...
### テスト結果
NO_TEST: テストコマンドが見つかりませんでした（npm test / pytest / go test のいずれも検出できず）

### 詳細
PR 作成は続行しますが、テストが存在しない可能性があります。人間によるレビューを推奨します。
```
