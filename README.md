...
## Agent Team（自律エージェントチーム）

GitHub Issue の自動処理・経営判断・インフラ監視などを行う自律エージェントチームです。
Leadership / Business / Ops / Dev の4つの Division が連携して動作します。

### 前提条件

| ツール | 必須？ | 用途 | インストール |
|--------|--------|------|-------------|
| [Claude Code](https://claude.ai/code) | ✅ 必須 | エージェント実行環境 | 公式サイトからインストール |
| GitHub CLI (`gh`) | ✅ 必須 | Issue / PR 操作 | `brew install gh` → `gh auth login` |
| Git | ✅ 必須 | リポジトリ操作 | 通常インストール済み |
| Node.js 18+ | ⭕️ MCP利用時のみ | MCP サーバー | [nodejs.org](https://nodejs.org/) |
...
mkdir -p ~/.claude/skills/
cp -r claude-hurikaeri/skills/agent-team ~/.claude/skills/
# ※ skills/agent-team が存在しない場合は ~/.claude/skills/ 以下に
#    agent-team ディレクトリを作成し、各 SKILL.md を配置してください。

# 3. GitHub CLI の認証（未実施の場合）
gh auth login
...
### 各 Division の概要と起動スケジュール

| Division | 役割 | 起動スケジュール |
|----------|------|----------------|
| Leadership | 経営判断・優先度設定・Issue 起票 | 毎日 8:57 |
| Business | 市場調査・競合分析・レポート生成 | 毎週月曜 9:07 |
| Ops | インフラ監視・アラート対応 | 15分ごと |
| Dev | Issue 自動実装・PR 作成 | 毎時 7分 |

### Division 個別インストール

特定の Division だけを使いたい場合は、対応する SKILL.md だけをインストールできます。

```bash
# Leadership Division のみ
cp -r claude-hurikaeri/skills/agent-team ~/.claude/skills/

# Claude Code で直接呼び出す場合
/leadership-division
/business-division
/ops-division
/dev-division
...
# 30分一時停止
/agent-team pause 30m
```

### よくある質問

**Q: Agent Team が動作しない場合は？**
A: 以下を確認してください：
- `gh auth status` で GitHub CLI が認証済みか確認
- `~/.claude/skills/agent-team/control.md` が存在するか確認
- `/agent-team status` でステータスを確認
- セッションを再開した場合は `/agent-team start` を再実行（CronJob はセッション内でのみ有効）

**Q: CronJob はセッションをまたいで動作しますか？**
A: いいえ。CronJob は Claude Code セッション内でのみ有効です。セッション再開時に `/agent-team start` を再実行してください。

**Q: 特定の Division だけを手動実行できますか？**
A: はい。Claude Code で `/dev-division`、`/ops-division` などと直接呼び出せます。
